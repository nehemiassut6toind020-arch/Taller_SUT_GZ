...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const { data: profile } = await supabaseClient.from('profiles').select('rol').eq('id', session.user.id).single();
...
/* ==========================================================
   CONTROLES DINÁMICOS DE FORMULARIO
   ========================================================== */
...
async function cargarClientes() {
  ...
  const opcionesClientes = soloClientes.length === 0 
    ...
    : '<option value="">Seleccione cliente...</option>' + soloClientes.map(c => `<option value="${c.id}">${c.nombre_completo}</option>`).join('');
...
async function guardarCliente(e) {
  ...
  if (editId) {
    ...
    if (!data || data.length === 0) {
      alert('Atención: La base de datos no devolvió ningún registro modificado. Verifique las políticas RLS en Supabase.');
      return;
    }
...
async function eliminarCliente(id) {
  ...
  if (error || (data && !data.success)) alert('Error al eliminar cliente.');
  else { alert('Cliente eliminado.'); await cargarClientes(); await cargarVehiculos(); }
}

/* ==========================================================
   CRUD VEHÍCULOS Y FILTRADO EN CASCADA
   ========================================================== */
...
function filtrarVehiculosPorCliente() {
  const clienteId = document.getElementById('selectClienteServicio').value;
  ...
  const vehiculosCliente = vehiculosGlobal.filter(v => v.id_propietario === clienteId);
...
async function guardarVehiculo(e) {
  ...
  if (editId) {
    const { error } = await supabaseClient.from('vehiculos').update({ id_propietario, marca, linea, modelo, placa, color }).eq('id', editId);
    if (error) alert('Error: ' + error.message);
    else { alert('Vehículo actualizado'); limpiarFormVehiculo(); await cargarVehiculos(); }
  } else {
    const { error } = await supabaseClient.from('vehiculos').insert([{ id_propietario, marca, linea, modelo, placa, color }]);
    if (error) alert('Error: ' + error.message);
    else { alert('Vehículo guardado'); limpiarFormVehiculo(); await cargarVehiculos(); }
  }
}
...
async function eliminarVehiculo(id) {
  ...
  if (error) alert('Error: ' + error.message);
  else { alert('Vehículo eliminado'); await cargarVehiculos(); }
}
...
async function registrarServicioYGenerarPDF(e) {
  ...
  const kilometraje_actual = parseInt(document.getElementById('kmActual').value);
  ...
  const proxima_fecha_servicio = document.getElementById('proximaFecha').value || null;

  // Datos Aceite Motor
  const aplico_aceite_motor = document.getElementById('chkAceiteMotor').checked;
  const tipo_aceite_motor = aplico_aceite_motor ? document.getElementById('tipoAceiteMotor').value.trim() : null;
  const km_proximo_motor = aplico_aceite_motor ? parseInt(document.getElementById('proximoKmMotor').value) : null;

  // Datos Aceite Caja
  const aplico_aceite_caja = document.getElementById('chkAceiteCaja').checked;
  const tipo_aceite_caja = aplico_aceite_caja ? document.getElementById('tipoAceiteCaja').value.trim() : null;
  const km_proximo_caja = aplico_aceite_caja ? parseInt(document.getElementById('proximoKmCaja').value) : null;
  ...
  const payload = {
    ...
    costo_total,
    detalles_trabajo,
    proxima_fecha_servicio,
    aplico_cambio_aceite: aplico_aceite_motor,
    tipo_aceite: tipo_aceite_motor,
    kilometraje_proximo_servicio: km_proximo_motor,
    aplico_aceite_caja,
    tipo_aceite_caja,
    km_proximo_caja
  };
...
function generarPDF(servicio) {
  ...
  const fechaServicio = servicio.fecha ? new Date(servicio.fecha).toLocaleDateString() : new Date().toLocaleDateString();
  ...
  // Filas dinámicas de la tabla de servicios
  const filasTabla = [
    ['Tipo de Servicio Principal', servicio.tipo_servicio || 'Mantenimiento General'],
    ['Kilometraje al Ingreso', `${servicio.kilometraje_actual} KM`]
  ];

  if (servicio.aplico_cambio_aceite) {
    filasTabla.push(['Aceite de Motor', `APLICADO (${servicio.tipo_aceite || 'N/A'})`]);
    filasTabla.push(['Próx. Cambio Aceite Motor', `${servicio.kilometraje_proximo_servicio || 'N/A'} KM`]);
  } else {
    filasTabla.push(['Aceite de Motor', 'NO APLICADO / NO REQUERIDO']);
  }

  if (servicio.aplico_aceite_caja) {
    filasTabla.push(['Aceite de Caja / Transmisión', `APLICADO (${servicio.tipo_aceite_caja || 'N/A'})`]);
    filasTabla.push(['Próx. Cambio Aceite Caja', `${servicio.km_proximo_caja || 'N/A'} KM`]);
  } else {
    filasTabla.push(['Aceite de Caja / Transmisión', 'NO APLICADO / NO REQUERIDO']);
  }

  if (servicio.proxima_fecha_servicio) {
    filasTabla.push(['Próxima Visita Recomendada', servicio.proxima_fecha_servicio]);
  }
  ...
  doc.save(`Ficha_Servicio_${v.placa || 'Vehiculo'}_${servicio.kilometraje_actual}KM.pdf`);
}